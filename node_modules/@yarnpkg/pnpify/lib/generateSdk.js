...
exports.getDisplayName = (name) => startCase_1.default(name).split(` `).map(word => capitalize_1.default(word)).join(` `);
exports.validateIntegrations = (integrations) => {
...
class IntegrationsFile {
    ...
    async persist(dir) {
        ...
        await fslib_1.xfs.mkdirpPromise(fslib_1.ppath.dirname(path));
...
class Wrapper {
    ...
    async writeManifest() {
        ...
        await fslib_1.xfs.writeFilePromise(absWrapperPath, JSON.stringify({
            name: this.name,
...
exports.generateSdk = async (pnpApi, { requestedIntegrations, preexistingIntegrations }, { report, onlyBase, verbose, configuration }) => {
    ...
    if (fslib_1.xfs.existsSync(oldTargetFolder) && !fslib_1.xfs.lstatSync(oldTargetFolder).isSymbolicLink()) {
        report.reportWarning(core_1.MessageName.UNNAMED, `Cleaning up the existing SDK files in the old ${configuration.format(exports.OLD_SDK_FOLDER, core_1.FormatType.PATH)} folder. You might need to manually update existing references outside the ${configuration.format(`.vscode`, core_1.FormatType.PATH)} folder (e.g. .gitignore)...`);
    ...
    await report.startTimerPromise(`Generating SDKs inside ${configuration.format(exports.SDK_FOLDER, core_1.FormatType.PATH)}`, async () => {
        const skipped = [];
        for (const [pkgName, generateBaseWrapper] of base_1.BASE_SDKS) {
            ...
            if (topLevelInformation.packageDependencies.has(pkgName)) {
                report.reportInfo(core_1.MessageName.UNNAMED, `${chalk_1.default.green(`✓`)} ${displayName}`);
        ...
        await report.startTimerPromise(`Generating settings`, async () => {
            for (const integration of allIntegrations) {
                if (preexistingIntegrations.has(integration)) {
                    report.reportInfo(core_1.MessageName.UNNAMED, `${chalk_1.default.green(`✓`)} ${exports.getDisplayName(integration)} (updated 🔼)`);
                }
                else {
                    report.reportInfo(core_1.MessageName.UNNAMED, `${chalk_1.default.green(`✓`)} ${exports.getDisplayName(integration)} (new ✨)`);
                }
            }
        });
    }
