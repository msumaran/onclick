...
const clientLandingReducer = (state = initialState, { type, payload }) => {

    const st = Object.assign({}, state)

    switch(type) {
        case 'CLIENT_LANDING_LOAD_FROM_DB':
            ...
            st.loaded = true
            break
        case 'CLIENT_LANDING_SAVE_DRAFT_TO_DB_START':
            st.saveDraftStatus = 'saving'
            st.code = payload.code
            break
        case 'CLIENT_LANDING_SAVE_DRAFT_TO_DB_END':
            st.saveDraftStatus = 'saved'
            break
        case 'CLIENT_LANDING_SAVE_DRAFT_TO_DB_ERROR':
            st.saveDraftStatus = 'error'
            break
        case 'CLIENT_LANDING_PUBLISH_START':
            st.publishStatus = 'publishing'
            st.code = payload.code
            break
        case 'CLIENT_LANDING_PUBLISH_END':
            st.publishStatus = 'published'
            st.published = true
            break
        case 'CLIENT_LANDING_PUBLISH_ERROR':
            st.publishStatus = 'error'
            break
        case 'CLIENT_LANDING_SAVE_SEO_START':
            st.saveSeoStatus = 'saving'
            break
        case 'CLIENT_LANDING_SAVE_SEO_END':
            st.saveSeoStatus = 'saved'
            st.seo = payload
            break
        case 'CLIENT_LANDING_SAVE_SEO_ERROR':
            st.saveSeoStatus = 'error'
            break
        case 'CLIENT_LANDING_SAVE_MESSAGES_START':
            st.saveMessagesStatus = 'saving'
            break
        case 'CLIENT_LANDING_SAVE_MESSAGES_END':
            st.saveMessagesStatus = 'saved'
            st.seo = payload
            break
        case 'CLIENT_LANDING_SAVE_MESSAGES_ERROR':
