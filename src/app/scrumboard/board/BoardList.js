...
function BoardList(props) {
    ...
    if (props.list.cards) {
        props.list.cards = _.keyBy(props.list.cards, 'id');
        props.list.cards = _.toArray(props.list.cards);
        ...
        <Draggable draggableId={props.list.id} index={props.index} type="list">
			{(provided, snapshot) => (
				<div ref={provided.innerRef} {...provided.draggableProps}>
					<Card
						className={clsx(classes.list, 'w-256 sm:w-320 mx-8 sm:mx-12 max-h-full flex flex-col bgbody')}
						square
								...
								<Droppable droppableId={props.list.id} type="card" direction="vertical">
									{_provided => (
										<div ref={_provided.innerRef} className="flex flex-col w-full h-full p-16">
											{props.list.cards && props.list.cards.map((card, index) => (
												<BoardCard
													key={card.id}
													card={card}
													index={index}
