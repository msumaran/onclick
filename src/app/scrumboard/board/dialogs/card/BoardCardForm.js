...
import LabelModel from 'app/scrumboard/model/LabelModel';
import * as Actions from 'app/scrumboard/store/actions/index';
...
function BoardCardForm(props) {
	...
	const list = useSelector(({ scrumboardApp }) => scrumboardApp.card.list);

	
	
	
	const { form: cardForm, handleChange, setForm, setInForm } = useForm(card);
	const updateCard = useDebounce((boardId, newCard, list) => {
		dispatch(Actions.updateCard(boardId, { ...newCard }, list));
	}, 600);
	const dueDate = cardForm && cardForm.due ? moment(cardForm.due).format(moment.HTML5_FMT.DATE) : '';

	useUpdateEffect(() => {
		updateCard(board.id, cardForm, list);
	...
	return (
		<>
					...
					<Toolbar className="flex w-full overflow-x-auto px-8 sm:px-16">
						<div className="flex flex-1">
							...
							*/}
							...
							<OptionsMenu onRemoveCard={() => dispatch(Actions.removeCard(board.id, list.id, cardForm.id))} />
						</div>
			...
			<DialogContent className="p-16 sm:p-24 bgblack">
